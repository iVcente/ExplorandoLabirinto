...
public class LabirintoV2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Número de linhas do labirinto: " + numLinhas);
        System.out.println("Número de colunas do labirinto: " + numColunas);
    ...
    private static void visitaVizinhos(int posAtualLinha, int posAtualColuna){
        List<Direcao> direcoesPossiveis = new ArrayList<>();
        Direcao norte = new Direcao(-1,0);
        Direcao sul = new Direcao(1,0);
        Direcao leste = new Direcao(0,1);
        Direcao oeste = new Direcao(0,-1);
        direcoesPossiveis.add(norte);
        direcoesPossiveis.add(sul);
        direcoesPossiveis.add(leste);
        direcoesPossiveis.add(oeste);
        ...
        * Oeste: (0, -1) // Esquerda
        */
        for (int i = 0; i < direcoesPossiveis.size(); i++){ // Percorrendo as quatro direcoes possiveis
 
            int vizinhoLinha = posAtualLinha + direcoesPossiveis.get(i).getLinha(); // Linha do vizinho sera igual a linha da posicao atual mais linha das direcoes possiveis, mudadas a cada iteracao
            int vizinhoColuna = posAtualColuna + direcoesPossiveis.get(i).getColuna(); // Coluna do vizinho sera igual a coluna da posicao atual mais coluna das direcoes possiveis, mudadas a cada iteracao

            if (posicaoValida(vizinhoLinha, vizinhoColuna) == false)
                continue;
    ...
    private static boolean posicaoValida(int vizinhoLinha, int vizinhoColuna){
        if (vizinhoLinha >= 0 || vizinhoColuna >= 0) // (!(vizinhoLinha < 0 || vizinhoColuna < 0))
            if (vizinhoLinha < numLinhas || vizinhoColuna < numColunas) // (!(vizinhoLinha >= numLinhas || vizinhoColuna >= numColunas))
                if (matrizVisitados[vizinhoLinha][vizinhoColuna] == false) // (!(matrizVisitados[vizinhoLinha][vizinhoColuna]))
                    if (matrizLabirinto[vizinhoLinha][vizinhoColuna] != '#') // (!(matrizLabirinto[vizinhoLinha][vizinhoColuna] == '#'))
                        return true;
